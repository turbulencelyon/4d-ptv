...
CMatrix::CMatrix(int rows, int cols) : m_rows(rows), m_cols(cols) {
...
void CMatrix::GetInput() {
    std::cin >> *this;
}

void CMatrix::FillSimulatedInput() {
    static int factor1 = 1, factor2 = 2;
    std::cout << "\n\nEnter Input For rows: "
    << m_rows << " Cols: " << m_cols << "\n";
    for (int i = 0; i < m_rows; i++)
    {
        for (int j = 0; j < m_cols; j++)
...
std::istream& operator >>(std::istream &is, CMatrix &m) {
    std::cout << "\n\nEnter Input For Matrix, Rows: " << m.m_rows << " Cols: " << m.m_cols << "\n";
...
std::ostream& operator <<(std::ostream &os, const CMatrix &m) {
    os << "Rows: " << m.m_rows << " Cols: " << m.m_cols << "\n";
    for (int i = 0; i < m.m_rows; i++)
    {
        os << " | ";
        for (int j = 0; j < m.m_cols; j++)
